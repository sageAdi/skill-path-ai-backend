...
Make sure to provide exactly 4 diverse transition options that cover different career directions (e.g., technical advancement, lateral moves, specialization, leadership).`;
            const systemPrompt = 'You are a career counselor expert specializing in tech career transitions. Provide realistic, actionable career transition suggestions based on the current role. Always return valid JSON with exactly 4 transitions.';
            const content = await this.ollamaRequest(prompt, systemPrompt, true);
            if (!content) {
                throw new Error('Empty response from Ollama');
            }
