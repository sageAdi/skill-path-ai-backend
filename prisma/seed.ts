...
import { PrismaClient, SkillDifficulty, type Skill } from '@prisma/client';
import { PrismaPg } from '@prisma/adapter-pg';
import { Pool } from 'pg';

// Initialize PrismaClient with adapter (matching PrismaService setup)
...
const skills = [
  // Web Development - Frontend
  {
    ...
    description:
      'Learn the basics of HTML structure, tags, and semantic markup',
  ...
  },
  {
    name: 'CSS Styling',
    description:
      'Master CSS for styling web pages, layouts, and responsive design',
    difficulty: SkillDifficulty.BEGINNER,
    category: 'Web Development',
  },
  {
    ...
    description:
      'Learn JavaScript fundamentals: variables, functions, and control flow',
    difficulty: SkillDifficulty.BEGINNER,
    category: 'Web Development',
  },
  {
  ...
  },
  {
    ...
    description:
      'Master advanced React patterns, performance optimization, and state management',
  ...
  // Software Engineering
  {
    ...
    description:
      'Master OOP principles: classes, inheritance, and polymorphism',
    difficulty: SkillDifficulty.BEGINNER,
...
async function main() {
  ...
  // Add some prerequisites (example relationships)
  // HTML and CSS are prerequisites for JavaScript
  ...
  const nodeSkill = createdSkills.find(
    (s) => s.name === 'Node.js Fundamentals',
  );
  ...
  const mlSkill = createdSkills.find(
    (s) => s.name === 'Machine Learning Fundamentals',
  );
  const gitSkill = createdSkills.find(
    (s) => s.name === 'Git & Version Control',
  );
  const dockerSkill = createdSkills.find((s) => s.name === 'Docker');

  const prerequisites: Array<{ skillId: string; prerequisiteId: string }> = [];

  // JavaScript requires HTML and CSS
  if (htmlSkill && cssSkill && jsSkill) {
    prerequisites.push(
      { skillId: jsSkill.id, prerequisiteId: htmlSkill.id },
      { skillId: jsSkill.id, prerequisiteId: cssSkill.id },
    );
  }
  ...
  // Machine Learning requires Python, Pandas, and NumPy
  if (pythonSkill && pandasSkill && numpySkill && mlSkill) {
    prerequisites.push(
      { skillId: mlSkill.id, prerequisiteId: pythonSkill.id },
      { skillId: mlSkill.id, prerequisiteId: pandasSkill.id },
      { skillId: mlSkill.id, prerequisiteId: numpySkill.id },
    );
  }
  ...
  for (const prereq of prerequisites) {
    try {
      ...
      const prereqSkill = createdSkills.find(
        (s) => s.id === prereq.prerequisiteId,
      );
